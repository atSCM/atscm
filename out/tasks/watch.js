  ...
  handleFileChange(path, root, stats) {
    return new Promise(resolve => {
      if (!this._pulling && _AtviseFile2.default.normalizeMtime(stats.mtime) > this._lastPull) {
        ...
        const source = (0, _src2.default)((0, _path.join)(root, path), { base: root, recursive: false });
    ...
    return new Promise(resolve => {
      if (!this._pushing) {
        if (readResult.nodeId.toString() !== this._lastPushed) {
          ...
          const stream = new _NodeStream2.default([readResult.nodeId], { recursive: false });

          new _PullStream2.default(stream).on('finish', () => {
            this._pulling = false;
