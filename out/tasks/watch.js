...
var _AtviseFile = _interopRequireDefault(require("../lib/server/AtviseFile"));
...
class WatchTask {
  ...
  handleFileChange(path, root, stats) {
    return new Promise(resolve => {
      if (!this._pulling && _AtviseFile.default.normalizeMtime(stats.mtime) > this._lastPull) {
        this._pushing = true;

        _gulplog.default.info(path, 'changed');

        const source = (0, _src.default)((0, _path.join)(root, path), {
          base: root,
          recursive: false
        });
        new _PushStream.default(source).on('data', file => this._lastPushed = file.nodeId.toString()).on('end', () => {
          this._pushing = false;
    ...
    return new Promise(resolve => {
      if (!this._pushing) {
        if (readResult.nodeId.toString() !== this._lastPushed) {
          ...
          _gulplog.default.info(readResult.nodeId.toString(), 'changed'); // FIXME: Reuse read value


          const stream = new _NodeStream.default([readResult.nodeId], {
            recursive: false
          });
          new _PullStream.default(stream).on('finish', () => {
            this._pulling = false;
            this._lastPull = _AtviseFile.default.normalizeMtime(readResult.mtime);
            this.browserSyncInstance.reload();
            resolve(true);
          });
        } else {
