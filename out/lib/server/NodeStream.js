      ...
      this.session.browse({
        ...
        if (err) {
          if (err.message === 'Transaction has timed out') {
            const tryNo = retry || 1;
            _gulplog2.default.warn(`Timeout while browsing. Retrying... (${tryNo})`, nodeId.toString());

            if (retry && retry === this.maxRetries) {
              reject(new Error(`Browsing ${nodeId.toString()} failed: Timeout (${promise.retry}x)`));
            } else {
              this.browseNode(nodeId, tryNo + 1).then(resolve, reject);
            }
          } else {
