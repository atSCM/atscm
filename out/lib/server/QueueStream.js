    ...
    this.processChunk(chunk, (err, statusCode, onSuccess) => {
      ...
      if (err) {
        ...
        if (process.env.CONTINUE_ON_FAILURE === 'true') {
          _gulplog2.default.error(`FAILURE: ${message}`);
        } else {
          this.emit('error', Object.assign(err, { message }));
        }
      } else if (statusCode !== _nodeOpcua.StatusCodes.Good) {
...
exports.default = QueueStream; /* Needed as long as https://github.com/gajus/eslint-plugin-jsdoc/issues/56 is open */
