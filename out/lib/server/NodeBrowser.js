...
class NodeBrowser {
    ...
    return new Promise((resolve, reject) => {
      ...
      return new Promise((resolve, reject) => {
        ...
        this._session.read(toRead, (err, _, [{
          ...
          if (err) return reject(err); // FIXME: valueRank -2 (Any) and -3 (ScalarOrOneDimension) are not handled properly here
  ...
  _browse({
    ...
    return new Promise((resolve, reject) => {
      this._session.browse({
        ...
        references
      }] = []) => err ? reject(err) : resolve(references));
    });
  }
  /**
   * Browses a node.
 ...
 - You can force pulling them by setting Atviseproject.preserveSortOrderNodes.`);
  ...
  async _push(node) {
    ...
    this._handled.set(node.id.value, 'processing'); // eslint-disable-next-line no-param-reassign


    node.value = (await this._readValue(node)) || node.value; // TODO: Remove additional properties (children, ...) for better memory-usage

    const originalId = node.id.value;
    await this._handleNode(node);
    this._pushed += 1; // Do not proceed if queue is stopped (because an error occured)
    ...
    this.ensureHandled.delete(originalId); // Handle dependencies
  ...
  async browse(nodeIds) {
    ...
    this._session = await _Session.default.create(); // Add source nodes

    const nodes = await this._getSourceNodes(nodeIds);
    this.queue.addAll(nodes.map(node => () => this._process(node))); // Queue error handling
