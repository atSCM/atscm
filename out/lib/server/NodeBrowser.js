...
class NodeBrowser {
  ...
  async _push(node) {
    ...
    this._handled.set(idValue, true); // Handle dependencies
  ...
  _hasDependencies(node) {
    ...
    for (const references of node.references.values()) {
      for (const reference of references) {
        if ( // !this._handled.get(reference) &&
        !this._isExternalReference(reference) && !this._ignoreNodesRegExp.test(reference)) {
          dependencyCount++;
