...
class NodeBrowser {
  ...
  constructor({ nodes, ignoreNodes, recursive /* : { nodes: NodeId[] } */ } = {}) {
    ...
    this._sourceNodesRegExp = new RegExp(`^(${nodes.map(({ value }) => `${value.replace(/\./g, '\\.')}`).join('|')})`);
    ...
    _Session2.default.create().then(session => this._session = session).then(() => this._getSourceNodes()).then(() => this._browseNext()).catch(err => {
  ...
  disconnect() {
    ...
    return _Session2.default.close(this._session);
  }
  ...
  async _getSourcePaths(nodes) {
    ...
    const browsePath = async items => {
      ...
      const results = await this._browse(items.map(({ nodeId }) => ({
        nodeId,
        browseDirection: _nodeOpcua.browse_service.BrowseDirection.Inverse,
        resultMask: 63
      })));
      ...
      results.forEach((result, i) => {
        ...
        for (const reference of result.references) {
          if (HierachicalReferencesTypeIds.has(reference.referenceTypeId.value)) {
            ...
            if (reference.nodeId.value !== _nodeOpcua.ObjectIds.RootFolder) {
  ...
  async _browseSourcePaths(paths, targets) {
    ...
    const browsePaths = async () => {
      ...
      remainingPaths.forEach((p, index) => {
        if (p.length) {
          browse.push({ nodeId: p.shift(), index });
        }
      });
  ...
  async _browseNext() {
    ...
    _gulplog2.default.debug('browsing', browseCount, 'nodes...');
    ...
    results.forEach((result, i) => {
      ...
      result.references.forEach(reference => {
        ...
        Object.setPrototypeOf(reference.nodeId, _NodeId2.default.prototype);

        if (HierachicalReferencesTypeIds.has(reference.referenceTypeId.value) && isChildReference(node, reference) && !this._ignoreNodesRegExp.test(reference.nodeId.value)) {
