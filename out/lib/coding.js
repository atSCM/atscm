...
const toRawValue = {
  ...
  [_variant.DataType.StatusCode]: ({ name }) => name,
  [_variant.DataType.QualifiedName]: ({ namespaceIndex, name }) => ({ namespaceIndex, name }),
  [_variant.DataType.LocalizedText]: ({ text, locale }) => ({ text, locale }),
  ...
  [_variant.DataType.Variant]: ({ dataType, arrayType, value, dimensions }) => ({
    dataType,
    arrayType,
 ...
 */
function encodeVariant({ value, dataType, arrayType }) {
  if (value === null) {
    return Buffer.from([]);
  }
...
const toNodeValue = {
  ...
  [_variant.DataType.Variant]: ({ dataType, arrayType, value, dimensions }) => new _variant.Variant({
    dataType,
 ...
 */
function decodeVariant(buffer, { dataType, arrayType }) {
