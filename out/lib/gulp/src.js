...
exports.SourceStream = exports.SourceBrowser = exports.FileNode = undefined;
exports.default = src;
...
class SourceBrowser {
  ...
  constructor({ path, base, ignoreNodes, recursive }) {
     ...
     */
    this._sourceNodesRegExp = new RegExp(`^(${_ProjectConfig2.default.nodes.map(({ value }) => `${value.replace(/\./g, '\\.')}`).join('|')})`);

    /**
     * A regular expression matching all nodes specified in {@link ProjectConfig.nodes}.
     */
    this._ignoreNodesRegExp = new RegExp(`^(${ignoreNodes || _ProjectConfig2.default.ignoreNodes.map(n => n.value).join('|')})`);
  ...
  _isRootNodePath(path) {
    const name = (0, _path2.relative)(this._base, path);
