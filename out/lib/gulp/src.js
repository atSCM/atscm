...
class SourceBrowser {
  ...
  _processNode(node) {
    ...
    if (!node._waitingFor) {
      const deps = Object.entries(node.references).filter(([key]) => key !== 'toParent' && !hierarchicalReferencesTypeNames.has(key)).reduce((result, [, ids]) => result.concat(ids.filter(id => !this._pushed.has(id))), []).filter(id => {
        if (typeof id === 'number') {
          // OPC-UA node
          return false;
        }
      ...
      if (depending) {
        depending.forEach(dep => {
          ...
          if (!dep.node.waitingFor.size) {
            ...
            return this._pushNode(_objectSpread({}, dep, {
          ...
          return _gulplog.default.debug('Still waiting', dep.node.nodeId, Array.from(dep.node.waitingFor));
        });
      } // eslint-disable-next-line no-param-reassign
