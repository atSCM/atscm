...
class SourceBrowser {
  ...
  constructor({
    ...
    this._rootNodePaths = new Set(_ProjectConfig.default.nodes.map(n => (0, _path2.join)(...n.value.split('.'), '.Object.json')));
    /**
     * A regular expression matching all nodes specified in {@link ProjectConfig.nodes}.
     */
  ...
  _isRootNodePath(path, parentPath) {
    if (parentPath === this._base) {
      return true;
    }

    const name = (0, _path2.relative)(this._base, path);

    if (this._rootNodePaths.has(name)) {
      return true;
