...
var _AddReferencesStream = require('../server/AddReferencesStream');

var _AddReferencesStream2 = _interopRequireDefault(_AddReferencesStream);
...
class PushStream {
  ...
  constructor(srcStream) {
    ...
    const addReferencesStream = new _AddReferencesStream2.default();
    const writeStream = new _WriteStream2.default(createStream, addReferencesStream);
    ...
    return _Transformer2.default.applyTransformers(srcStream.pipe(mappingStream), _ProjectConfig2.default.useTransformers.concat(new _Newlines2.default()), _Transformer.TransformDirection.FromFilesystem).pipe(writeStream).pipe(createStream).pipe(addReferencesStream).on('finish', () => {
      if (_gulplog2.default.listenerCount('info') > 0) {
        _readline2.default.cursorTo(process.stdout, 0);
