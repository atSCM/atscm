...
class WriteStream extends _stream.Writable {
  ...
  async _writeNode(node) {
    ...
    const writeOps = []; // Rename nodes specified in the rename config
    ...
    if (node.nodeClass === _nodeclass.NodeClass.Variable) {
      if (node.value) {
        if (!node.value.noWrite) {
          if (this._performWrites) {
            writeOps.push(this._outputFile((0, _path.join)(this._base, dirPath.join('/'), node.fileName), (0, _coding.encodeVariant)(node.value)));
 ...
 - Resolve all conflicts inside '${renameConfigPath}' and run 'atscm pull' again`); // FIXME: Insert link to node ide conflict manual here once 1.0.0 is released.
