...
class WriteStream extends _stream.Writable {
  ...
  constructor(options) {
     ...
     * @type Promise<Object>
     */
    this._loadRenameConfig = (0, _fsExtra.readJson)(renameConfigPath).then(config => this._renameConfig = config).catch(() => _gulplog2.default.debug('No rename config file loaded'));
  ...
  async _writeNode(node) {
    ...
    if (rename && rename !== renameDefaultName) {
      ...
      _gulplog2.default.debug(`'${node.nodeId}' was renamed to '${rename}'`);
    ...
    if (!node._renamed && node.nodeId !== node.id.value) {
      _gulplog2.default.debug(`Resolved ID conflict: '${node.id.value}' should be renamed to '${node.nodeId}'`);
    }
    ...
    if (node.name.match(/:/)) {
      ...
      _gulplog2.default.debug(`Resolved ID conflict: '${before}' was renamed to safe name '${node.name}'`);
    }
    ...
    if (this._idMap.has(pathKey)) {
      if (this._parentHasIdConflict(node)) {
        _gulplog2.default.debug(`ID conflict: Skipping '${node.nodeId}'`);
      } else {
        _gulplog2.default.error(`ID conflict: '${node.nodeId}' conflicts with '${this._idMap.get(pathKey)}'`);
        ...
        if (existingRename) {
          if (existingRename === renameDefaultName) {
            ...
            _gulplog2.default.error(` - '${node.nodeId}' is present inside the rename file at './atscm/rename.json', but no name has been inserted yet.`);
          } else {
            // eslint-disable-next-line max-len
            _gulplog2.default.error(` - The name for '${node.nodeId}' inside './atscm/rename.json' is not unique.`);
          }

          _gulplog2.default.info(" - Edit the node's name and run 'atscm pull' again");
        } else {
          this._renameConfig[node.nodeId] = renameDefaultName;
          _gulplog2.default.info(` - '${node.nodeId}' was added to the rename file at './atscm/rename.json'`);
          _gulplog2.default.info("Edit it's name and run 'atscm pull' again.");
        }
      }
  ...
  _writev(nodes, callback) {
    ...
    this._loadRenameConfig.then(() => Promise.all(nodes.map(({ chunk }) => this._writeNode(chunk)))).then(() => callback()).catch(err => callback(err));
  }
  ...
  _final(callback) {
    if (this._discoveredIdConflicts) {
      _gulplog2.default.error(`Discovered ${this._discoveredIdConflicts} node id conflicts, results are incomplete.
 - Resolve all conflicts inside '${renameConfigPath}' and run 'atscm pull' again`);
    ...
    (0, _fsExtra.outputFile)(renameConfigPath, JSON.stringify(this._renameConfig, null, '  ')).then(callback).catch(callback);
  }
