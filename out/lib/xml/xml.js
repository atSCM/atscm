 ...
 */
['join', 'indexOf', 'lastIndexOf', 'forEach', 'map', 'reduce', 'reduceRight', 'filter', 'some', 'every', 'concat', 'slice'].forEach(function (methodName) {
...
var isNode = xml.isNode = function (node) {
    return typeof node === 'string' || node instanceof Comment || node instanceof CData || node instanceof Tag;
};
...
var is = xml.is = function (expr, node) {
    return expr === 'node()' && isNode(node) || expr === XML_TYPE.TEXT && typeof node === 'string' || expr === XML_TYPE.COMMENT && node instanceof Comment || expr === XML_TYPE.CDATA && node instanceof CData || node instanceof Tag && (expr === '*' || expr === XML_TYPE.ELEMENT || expr === node.name);
};
