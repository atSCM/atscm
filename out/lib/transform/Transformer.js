...
var _AtviseFile = _interopRequireDefault(require("../server/AtviseFile"));
...
class Transformer extends (0, _through.ctor)({
  objectMode: true
}) {
  /**
  ...
  _processError(err, chunk, callback, ...args) {
    if (err) {
      const id = (this.direction === TransformDirection.FromDB ? chunk.nodeId : chunk.relative) || chunk.toString(); // eslint-disable-next-line no-param-reassign

      err.message = `[${this.constructor.name}] ${err.message} (in ${id})`;

      if (process.env.CONTINUE_ON_FAILURE === 'true') {
        _gulplog.default.error(err.message);
  ...
  _transform(chunk, enc, callback) {
    if (!this.transformsReferenceConfigFiles && chunk instanceof _AtviseFile.default && chunk.isReferenceConfig) {
      callback(null, chunk);
      return;
    }
  ...
  static applyTransformers(stream, transformers, direction) {
    ...
    return (direction === TransformDirection.FromDB ? transformers : transformers.reverse()).reduce((prev, curr) => curr.applyToStream(prev, direction), stream);
  }
  /**
  ...
  inspect(depth, options) {
    ...
    return `${options.stylize(this.constructor.name, 'special')}${(0, _util.inspect)(this._options, newOptions).replace(/^{/, '<').replace(/}$/, '>').replace(/\n/, `\n${' '.repeat(this.constructor.name.length)}`)}`;
  }
