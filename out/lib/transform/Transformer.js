...
exports.TransformDirection = undefined;
 ...
 * @abstract
 */
class Transformer extends (0, _through.ctor)({ objectMode: true }) {
  ...
  _processError(err, chunk, callback, ...args) {
    if (err) {
      const id = (this.direction === TransformDirection.FromDB ? chunk.nodeId : chunk.relative) || chunk.toString();

      // eslint-disable-next-line no-param-reassign
      err.message = `[${this.constructor.name}] ${err.message} (in ${id})`;

      if (process.env.CONTINUE_ON_FAILURE === 'true') {
        _gulplog2.default.error(err.message);
        callback(null);
      } else {
        callback(err);
      }
  ...
  _transform(chunk, enc, callback) {
    if (!this.transformsReferenceConfigFiles && chunk instanceof _AtviseFile2.default && chunk.isReferenceConfig) {
      callback(null, chunk);
      return;
    }
