 ...
 * @abstract
 */
class PartialTransformer extends _Transformer.default {
  /**
  ...
  constructor(options = {}) {
    ...
    this.filter = (0, _gulpFilter.default)(file => this.shouldBeTransformed(file), {
      restore: true
    });
  }
  /**
  ...
  applyToStream(stream, direction) {
    ...
    return (this.applyToFilteredStream(filteredStream, direction) || filteredStream.pipe(this.withDirection(direction))).pipe(this.filter.restore);
  }
  /**
