...
class CombineFilesCache {
  ...
  gotAllFiles(file, callback) {
    ...
    if (!this._required[dirname]) {
      (0, _fs.readdir)(dirname, (err, files) => {
        if (err) {
          callback(err);
        } else {
          this._files[dirname] = {};
          this._required[dirname] = files.filter(name => name[0] !== '.').map(name => (0, _path.extname)(name));
...
class SplittingTransformer extends _PartialTransformer2.default {
  ...
  _flush(callback) {
    ...
    if (missingDirnames.length > 0) {
      Promise.all(missingDirnames.map(dirname => {
        ...
        const stem = (0, _path.basename)(dirname, (0, _path.extname)(dirname));
        const paths = missing.map(ext => (0, _path.join)(dirname, '/', `${stem}${ext}`));

        delete this._combineFilesCache._files[dirname];
        delete this._combineFilesCache._required[dirname];

        _gulplog2.default.debug('Loading', paths.length, 'required file(s)');

        return Promise.all(paths.map((path, i) => _AtviseFile2.default.read({
          cwd: firstFile.cwd,
          base: firstFile.base,
          path
        }).then(file => {
      ...
      })).then(files => _gulplog2.default.debug('Created', files.length, 'additional file(s)')).then(() => callback()).catch(err => callback(err));
    } else {
      callback();
    }
  }
