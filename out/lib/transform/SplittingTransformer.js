...
exports.default = exports.CombineFilesCache = void 0;

var _fs = require("fs");

var _path = require("path");

var _PartialTransformer = _interopRequireDefault(require("./PartialTransformer"));
...
class CombineFilesCache {
  ...
  gotAllFiles(node, callback) {
    if (this._waitingFor[node.relative]) {
      const wrapper = (0, _path.dirname)(node.relative);
      this._files[wrapper][(0, _path.extname)(node.relative)] = node;
    ...
    const wrapperExt = (0, _path.extname)(node.relative);
    const wrapperName = (0, _path.basename)(node.relative, wrapperExt);

    this._wrappers.set(node.relative, node);

    (0, _fs.readdir)(node.relative, (err, files) => {
      if (err) {
        callback(err);
      } else {
        this._files[node.relative] = {};
        this._required[node.relative] = files.filter(name => name[0] !== '.' && !name.endsWith(wrapperExt) && name.match(new RegExp(`^${wrapperName}\\..+`))).map(name => {
          this._waitingFor[(0, _path.join)(node.relative, name)] = true;
          return (0, _path.extname)(name);
        });
        callback(null);
      }
    });
...
exports.CombineFilesCache = CombineFilesCache;
...
class SplittingTransformer extends _PartialTransformer.default {
  ...
  shouldBeTransformed(node) {
    if (this.direction === _Transformer.TransformDirection.FromDB) {
      return false;
    }
  ...
  _flush(callback) {
    ...
    if (missingDirnames.length > 0) {
      callback(); // FIXME: Assert in push but not in watch task
