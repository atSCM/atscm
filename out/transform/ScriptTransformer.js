 ...
 */
class ScriptTransformer extends _XMLTransformer.default {
  /**
  ...
  transformFromDB(node, enc, callback) {
    this.decodeContents(node, (err, results) => {
      if (err) {
        ...
        if (!document) {
          _gulplog.default.warn(`Empty document at ${node.nodeId}`);
        }

        const config = {}; // Extract metadata
            ...
            if (child.type === 'element') {
              if (child.name === 'icon') {
                ...
                if (!['longrunning'].includes(child.name)) {
                  _gulplog.default.debug(`Generic metadata element '${child.name}' at ${node.nodeId}`);
                }
              }
            }
          });
        ...
        const code = (0, _xml.textContent)(codeNode) || ''; // Write config file
        ...
        this.push(configFile); // Write script file
