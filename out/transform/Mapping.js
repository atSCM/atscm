...
class MappingTransformer extends _Transformer2.default {
  ...
  transformFromDB(readResult, encoding, callback) {
    try {
      ...
      if (file.relative.match(/\.var\./)) {
        ...
        rc.contents = _buffer.Buffer.from(JSON.stringify({
  ...
  transformFromFilesystem(file, encoding, callback) {
    if (file.isDirectory()) {
      ...
      if (file.relative.match(/\.var\./)) {
        const rcPath = (0, _path.join)(file.dirname, `.${(0, _path.basename)(file.stem, '.var')}.rc`);

        (0, _fs.readFile)(rcPath, 'utf8', (err, data) => {
          try {
