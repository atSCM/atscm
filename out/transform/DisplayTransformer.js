...
var _sortJson = require('sort-json');

var _sortJson2 = _interopRequireDefault(_sortJson);
  ...
  transformFromDB(file, enc, callback) {
    this.decodeContents(file, (err, xmlObj) => {
      if (err) {
        ...
        _gulplog2.default.error(`Display ${file.nodeId}: Xml object is empty`);
        callback(null);
      } else {
        ...
        if (rootSVG) {
          try {
            ...
            configFile.contents = _buffer.Buffer.from(JSON.stringify((0, _sortJson2.default)(config), null, '  '));
        ...
        try {
          ...
          if (parameters && parameters.length > 0) {
            if (metadata.children[0]) {
              ...
              _gulplog2.default.error(`Display ${svgFile.nodeId}: Metadata tag is missing.`, 'Can not append parameters');
            }
          }
