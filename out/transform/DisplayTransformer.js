...
class DisplayTransformer extends _XMLTransformer.default {
        ...
        if (scriptTags.length) {
          scriptTags.forEach(script => {
            if (script.attributes && (script.attributes.src || script.attributes['xlink:href'])) {
              ...
              if (inlineScript) {
                _gulplog.default.warn(`'${node.id.value}' contains multiple inline scripts.`);
  ...
  createCombinedFile(node, sources, callback) {
    ...
    node.markAsResolved('nodeClass'); // Set dataType to DataType.XmlElement

    node.value.dataType = _variant.DataType.XmlElement;
    node.markAsResolved('dataType'); // Set arrayType to 'Scalar'

    node.value.arrayType = _variant.VariantArrayType.Scalar;
    node.markAsResolved('arrayType'); // Set type definition reference to 'VariableTypes.ATVISE.Display'
