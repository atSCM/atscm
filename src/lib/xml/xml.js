 ...
 * @constructor
 */
const NodeSet = xml.NodeSet = function NodeSet(nodes) {
...
NodeSet.prototype.find = function(expr) {
  let _expr = (typeof expr === 'string') ? expr.split('/') : expr,
...
NodeSet.prototype.text = function(keepArray) {
  const result = (function extractNSetText(nset) {
    let result = [];
 ...
 * @constructor
 */
const Comment = xml.Comment = function Comment(comment) {
 ...
 * @constructor
 */
const CData = xml.CData = function CData(chunk) {
  this.rawData = [];
  this.data = '';
  this.cached = true;
 ...
 * @returns {Boolean}
 */
const isNode = xml.isNode = function(node) {
